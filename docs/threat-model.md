...
| --- | --- | --- |
| Atlas host and environment modules | Declare state roots, volumes, host contract, Tailscale adapter, root-only Nix authority, persistent nspawn entry, split inspection/lifecycle control, control/environment slices, accounts, and the explicit DigitalOcean dogfood adapter | `nixos/modules/atlas-host.nix`, `nixos/modules/atlas-environments.nix`, `nixos/modules/atlas-digitalocean.nix` |
...
| Host-contract tests | Check contract intent, default absence of OpenSSH, the explicit bootstrap exception, generated state, anchored peer identity, persistent and concurrent user-namespaced entry, environment mutation, package installation, Btrfs seed/reset/snapshot/restore, explicit volume sharing and denial, root-only Nix, safe option evaluation, volume survival, and an encrypted x86 installed-disk lifecycle | `nixos/tests/host-contract.nix`, `nixos/tests/installed-host.nix`, `nixos/tests/module-evaluation.nix`, `tests/test_atlas_control.py` |
...
| Environment instance | Mutable Ubuntu root and environment-local owner state | declared environment plus pinned OCI base | writable Btrfs subvolume at `/var/lib/atlas/environments/<id>/rootfs`, reconstructed from a host-only read-only applied seed on reset | human-owner UID with passwordless environment-local sudo; privileged Atlas launcher and root-only lifecycle service | persistent user-namespaced systemd-nspawn, anchored cgroup identity, fixed entry sudo rule, pinned Ubuntu-native bootstrap packages, exact bootstrap-completion marker, external readiness marker, fail-closed managed-path validation, no-follow snapshot compatibility reads, COW root replacement; seed extraction capabilities are confined by a strict filesystem view to environment state and lifecycle locks | Normal-owner entry, Ubuntu-owned `/usr/bin/sudo` without Nix PAM paths, passwordless environment sudo, resettable home composition, package installation, reboot, snapshot, and reset passed the complete KVM host proof; shared-host networking, physical recovery, and broader Nix-store visibility remain degraded |
| Owner home volume | Conventional durable owner files | configured single owner plus per-environment `ownerHome` attachment | automatic sibling Btrfs subvolume mounted at `/home/<owner>`; environment-root directories overlay `~/.config`, `~/.cache`, `~/.local/bin`, and `~/.local/state` | environments explicitly receiving the owner home, plus host control | nspawn idmapped bind followed by nested resettable binds; descriptor-relative host preparation refuses symbolic links; durable source excluded from root snapshots and reset; reset and restore verify its device and inode | Reboot, cross-environment sharing, omission, reset, snapshot behavior, and final or intermediate symlink refusal passed the KVM host proof; reboot and reset survival also passed the encrypted installed-layout proof. Arbitrary dotfiles outside the declared paths remain durable by design; backup, ACLs, and durable-home snapshots are absent |
| Project volume | Durable source and artifacts | declared volume plus explicit environment mount | sibling Btrfs subvolume at `/var/lib/atlas/volumes/<id>/data`, mounted at `/home/owner/Projects` in the current proof | only environments with the declared mount, plus host control | nspawn bind with id mapping and per-environment declaration; excluded from root snapshots and reset | Nested mounting, sharing between two environments, omission from a third, reboot, reset, root restore, and generation rollback passed the KVM host proof. Encryption is absent on development and live adapters but KVM-proven for the installed layout; optional quotas, ACLs, durable-volume snapshots, and backup are absent |
