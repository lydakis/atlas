...
- The DigitalOcean dogfood profile relies on provider-managed volume encryption,
  ...
  key-only root OpenSSH, and current shared-host environment networking can
  reach cloud metadata unless an external control blocks it. The live host is
  now on its steady Tailscale-only profile, but do not place consequential
  credentials on it before private environment networking is proven.
