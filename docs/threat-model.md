...
contracts and a bounded NixOS spike. The spike defines Atlas state roots, a
protected control-plane resource slice, root-only Nix and Incus authority,
interactive or runtime-secret Tailscale enrollment, a live physical ISO,
resettable isolated-ID-map Incus environments, explicit volumes, and an
operator-only reset and snapshot path separated from the public inspection socket. It
...
| --- | --- | --- |
| Atlas host and environment modules | Declare state roots, volumes, host contract, Tailscale adapter, root-only Nix and Incus authority, persistent Incus entry, split inspection/lifecycle control, accounts, private environment networking, and the explicit DigitalOcean dogfood adapter | `nixos/modules/atlas-host.nix`, `nixos/modules/atlas-environments.nix`, `nixos/modules/atlas-digitalocean.nix` |
...
| Host-contract tests | Check contract intent, default absence of OpenSSH, the explicit bootstrap exception, host-bound environment identity, persistent Incus entry, environment mutation, package installation, Incus reset/snapshot/restore, explicit volume sharing and denial, private network configuration, root-only Nix and Incus authority, safe option evaluation, daemon recovery, volume survival, and an encrypted x86 installed-disk lifecycle | `nixos/tests/host-contract.nix`, `nixos/tests/installed-host.nix`, `nixos/tests/module-evaluation.nix`, `tests/test_atlas_control.py` |
| Flake outputs | Build VM test artifacts, instantiate the parameterized x86 installed-storage module in KVM, build a non-persistent live ISO, and evaluate bootstrap and steady DigitalOcean profiles plus a qcow2 image | `flake.nix` |
...
| DigitalOcean dogfood host | Persistent Atlas state | separately attached volume named `atlas-data` | provider-stable device mounted as Btrfs at `/var/lib/atlas` | host root and Atlas-declared environment mounts | explicit device name, Btrfs verification, separate root disk, and provider-managed volume encryption | Live Btrfs mount, two reboots, reset, and durable-volume persistence passed. Provider encryption is reported from the deployment contract rather than independently verified; the provider, account, and control plane remain trusted: `nixos/modules/atlas-digitalocean.nix`, `docs/nixos-spike.md` |
| DigitalOcean bootstrap | Temporary public host entry | explicit `bootstrapOpenSsh.enable`, root only, public key only | TCP 22 until Tailscale enrollment and switch to the steady profile | holders of a selected DigitalOcean metadata SSH key, subject to the external Cloud Firewall | OpenSSH account and authentication restrictions, host firewall, provider Cloud Firewall, explicit steady profile, and steady-activation removal of the metadata-written root key | Deliberate degraded bootstrap. The live dogfood transition restricted the listener to one source, proved Tailscale and provider recovery, disabled OpenSSH, removed the provider firewall, and verified both public SSH closure and root-key removal across reboot: `nixos/modules/atlas-host.nix`, `nixos/modules/atlas-digitalocean.nix`, `flake.nix`, `docs/nixos-spike.md` |
...
| Environment instance | Mutable Ubuntu root and environment-local owner state | declared environment plus content-bound Incus system image | Incus-managed root and dependent resettable volumes in the Atlas Btrfs pool; delete-and-recreate reset from the pinned image | human-owner UID with passwordless environment-local sudo; privileged Atlas launcher and root-only lifecycle service | persistent unprivileged Incus container, isolated ID map, no mutable profiles, disabled guest API, fixed entry sudo rule, host-only administrative socket, environment-bound public control proxy, private NIC ACL, pinned image hashes, Atlas-owned startup, local-daemon-only lifecycle calls | Normal-owner entry, environment sudo, resettable home composition, package installation, instance restart, snapshot, reset, listener identity, and daemon recovery pass the host proof. The controlled IPv4 connection matrix also passes across declaration changes and reboot. Physical recovery, live deployment revalidation, and broader Nix-store visibility remain open or degraded |
| Owner home volume | Conventional durable owner files | configured single owner plus per-environment `ownerHome` attachment | automatic sibling Btrfs subvolume mounted at `/home/<owner>` through a shifted Incus disk device; dependent Incus volumes overlay `~/.config`, `~/.cache`, `~/.local/bin`, and `~/.local/state` | environments explicitly receiving the owner home, plus host control | isolated ID maps plus Incus shifting; descriptor-relative host preparation refuses symbolic links; durable source excluded from instance snapshots and reset; reset and restore verify its Btrfs subvolume UUID, with device and inode as the non-Btrfs fallback | Cross-environment sharing, omission, reset, and snapshot behavior pass the KVM host proof; reboot and reset survival also passed the earlier encrypted installed-layout proof. Arbitrary dotfiles outside the declared paths remain durable by design; backup, ACLs, and durable-home snapshots are absent |
| Project volume | Durable source and artifacts | declared volume plus explicit environment mount | sibling Btrfs subvolume at `/var/lib/atlas/volumes/<id>/data`, mounted at `/home/owner/Projects` through a shifted Incus disk device | only environments with the declared mount, plus host control | per-environment Incus device declaration; excluded from instance snapshots and reset; reset and restore verify its Btrfs subvolume UUID, with device and inode as the non-Btrfs fallback | Sharing between two environments, omission from a third, instance restart, reset, and snapshot restore pass the KVM host proof. Encryption is absent on development and live adapters but KVM-proven for the installed layout; optional quotas, ACLs, durable-volume snapshots, and backup are absent |
...
- The DigitalOcean dogfood profile relies on provider-managed volume encryption,
  not an owner-held unlock key. Its bootstrap profile temporarily exposes
  key-only root OpenSSH. Earlier shared-host networking could reach cloud
  metadata unless an external control blocked it. The Incus private-network
  VM proof does not establish the current live deployment's guarantees;
  revalidate that deployment before placing consequential credentials on it.
...
| High hypothesis | The DigitalOcean bootstrap listener is reachable beyond the operator or remains active after enrollment | bootstrap profile plus a missing, broad, or stale provider firewall rule | public root-SSH attack surface and reliance on one long-lived key | root public-key authentication only, passwords disabled, environments excluded from OpenSSH, explicit steady profile | restrict TCP 22 to the operator's current IP before boot, verify Tailscale and provider-console recovery, switch to the OpenSSH-disabled profile, then remove the rule | One live transition passed and public SSH was verified closed; repeatability and external provider-policy drift remain untested |
...
| High hypothesis | An environment escapes through mounts, devices, capabilities, ptrace, namespaces, the kernel, or the Atlas control socket | hostile environment-root-capable process inside an Incus container | host root or another environment's data and authority | unprivileged isolated ID map, resettable private root, explicit disk devices, disabled guest API, no administrative socket, environment-bound public control proxy, private NIC ACL, exact entry sudo rule, root-only lifecycle socket | reduce mounted host surfaces, complete network and control fuzzing, test known container escape classes, qualify Incus and kernel patches quickly, consider microVMs for hostile multi-tenancy | Entry, host `/etc` isolation, distinct namespaces, listener-bound identity, administrative-socket absence, public reset denial, snapshot, and reset are tested in `nixos/tests/host-contract.nix`; comprehensive escape resistance is not proven |
...
| Medium hypothesis | A malicious environment exhausts resources and blocks inspection or recovery | interactive environment with excessive CPU, memory, tasks, resettable-root writes, durable-volume writes, logs, or recordings | host denial of service, loss of recovery headroom, and possible state loss | protected control-plane slice and a KVM-tested installed layout with fixed host and elastic Atlas logical volumes | enforce and runtime-test Incus CPU, memory, task, and optional storage limits; add log, cache, and recording limits; run full-disk and metadata-exhaustion pressure tests | Structural host-capacity separation exists, but the former per-environment systemd slice controls were removed with nspawn and equivalent Incus limits are not yet proven. Elastic roots deliberately have no default quota; physical exhaustion and recovery remain unproven |
| Medium hypothesis | A client label or logical agent identifier is trusted as the environment principal | capability-aware client or local control socket | confused-deputy access to another environment | host entry UIDs remain kernel-derived; Incus proxy listeners bind one declared environment identity on the host and expose only the public protocol; unknown callers and caller-supplied identity fail closed | apply the same observed-principal rule to every future grant, route, volume, and surface method and test proxy, namespace, and listener edge cases | Unit and booted-host identity and reset tests exist in `tests/test_atlas_control.py` and `nixos/tests/host-contract.nix` |
