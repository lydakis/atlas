...
| Environment instance | Mutable Ubuntu root and environment-local owner state | declared environment plus pinned OCI base | writable Btrfs subvolume at `/var/lib/atlas/environments/<id>/rootfs`, reconstructed from a host-only read-only applied seed on reset | human-owner UID with passwordless environment-local sudo; privileged Atlas launcher and root-only lifecycle service | persistent user-namespaced systemd-nspawn, anchored cgroup identity, fixed entry sudo rule, external readiness marker, fail-closed managed-path validation, no-follow snapshot compatibility reads, COW root replacement; seed extraction capabilities are confined by a strict filesystem view to environment state and lifecycle locks | Normal-owner entry, passwordless environment sudo, resettable home composition, package installation, reboot, snapshot, and reset passed the complete KVM host proof; shared-host networking, physical recovery, and Nix-store visibility remain degraded |
